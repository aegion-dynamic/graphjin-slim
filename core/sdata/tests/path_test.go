...
func tableNodeID(t testing.TB, s *DBSchema, name string) int32 {
	...
	id, ok := s.TableNodeID("public", name)
	if !ok {
...
func TestFKCyclesDetectSelfRecursiveFK(t *testing.T) {
	...
	if !slices.Contains(s.FKCyclesSelf(), selfID) {
		t.Fatalf("expected self-recursive employees FK, got %+v", s.FKCyclesSelf())
	}
	if len(s.FKCyclesComponents()) != 0 {
		t.Fatalf("self-recursive FK should not create a multi-table component: %+v", s.FKCyclesComponents())
	}
}
...
func TestFKCyclesDetectMultiTableCycle(t *testing.T) {
	...
	if len(s.FKCyclesComponents()) != 1 {
		t.Fatalf("expected one FK cycle component, got %+v", s.FKCyclesComponents())
	}
	got := tableNames(t, s, s.FKCyclesComponents()[0])
	want := []string{"a", "b", "c"}
...
func TestFKCyclesIgnoreSyntheticReverseEdges(t *testing.T) {
	...
	if len(s.FKCyclesSelf()) != 0 {
		t.Fatalf("unexpected self cycles: %+v", s.FKCyclesSelf())
	}
	if len(s.FKCyclesComponents()) != 0 {
		t.Fatalf("synthetic reverse traversal edges inflated SCCs: %+v", s.FKCyclesComponents())
	}
}
...
func BenchmarkFindPathDenseGraphWarm(b *testing.B) {
	s := densePathSchema(b, 80, 8)
	if _, err := s.FindPath("t0", "t79", ""); err != nil {
		b.Fatal(err)
	}
...
func BenchmarkFindPathDisconnectedComponents(b *testing.B) {
...
func BenchmarkFindPathWeightedDirectVersusRemoteHop(b *testing.B) {
	s := testSchema(b,
		testPK("a"), testFK("a", "b_id", "b"),
		testPK("b"), testFK("b", "c_id", "c"),
		testPK("c"),
