...
type (
	...
	DDLDialect      = schema.DDLDialect
	SchemaOperation = schema.SchemaOperation
	postgresDialect = schema.PostgresDialect
	sqliteDialect   = schema.SQLiteDialect
)

var (
	computeDiff       = schema.ComputeDiff
	ComputeDiff       = schema.ComputeDiff
	SupportsSchemaDDL = schema.SupportsSchemaDDL
	GenerateDiffSQL   = schema.GenerateDiffSQL
	parseTypeWithSize = schema.ParseTypeWithSize
)
