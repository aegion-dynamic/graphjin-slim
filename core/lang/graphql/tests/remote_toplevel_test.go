...
func TestTopLevelRemoteIsMarkedAsRemote(t *testing.T) {
	co, err := graphql.NewCompiler(newToplevelRemoteSchema(t), graphql.Config{DefaultLimit: 5, DisableAgg: true, DisableFuncs: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoteUnknownColumnStrictErrors(t *testing.T) {
	...
	co, err := graphql.NewCompiler(s, graphql.Config{DefaultLimit: 5, DisableAgg: true, DisableFuncs: true})
	if err != nil {
		t.Fatal(err)
	}
