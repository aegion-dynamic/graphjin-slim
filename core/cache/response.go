...
// FilesystemKeyRef builds a single key-level filesystem cache ref.
...
// Keep unexported aliases used inside this package.
func filesystemKeyRef(name, key string) RowRef       { return FilesystemKeyRef(name, key) }
func filesystemPrefixRef(name, prefix string) RowRef { return FilesystemPrefixRef(name, prefix) }
...
// StripCacheTrackingFields removes internal __gj_id fields from a JSON response.
func StripCacheTrackingFields(data []byte) ([]byte, error) {
	return stripCacheTrackingFields(data)
}
