...
const (
	compositeFKQueryMySQL              = introspection.CompositeFKQueryMySQL
	compositeFKQuerySQLite             = introspection.CompositeFKQuerySQLite
	compositeFKQueryOracle             = introspection.CompositeFKQueryOracle
	compositeFKQueryMSSQL              = introspection.CompositeFKQueryMSSQL
	compositeFKQuerySnowflakeOverrides = introspection.CompositeFKQuerySnowflakeOverrides
	snowflakeCompositeFKShowStmt       = introspection.SnowflakeCompositeFKShowStmt
	introspectionQueryTimeout          = introspection.IntrospectionQueryTimeout
)

var (
	GetDBInfo                        = introspection.GetDBInfo
	ParseClusteringKey               = introspection.ParseClusteringKey
	AutoSetPartitionFromClustering   = introspection.AutoSetPartitionFromClustering
	autoSetPartitionFromClustering   = introspection.AutoSetPartitionFromClustering
	DiscoverCompositeFKs             = introspection.DiscoverCompositeFKs
	hasCompositeFKCandidates         = introspection.HasCompositeFKCandidates
	snowflakeSkipClusteringDiscovery = introspection.SnowflakeSkipClusteringDiscovery
	discoverClusteringKeys           = introspection.DiscoverClusteringKeys
	isInList                         = introspection.IsInList
	inferDefaultSchema               = introspection.InferDefaultSchema
	inferViewPKsFromBaseTables       = introspection.InferViewPKsFromBaseTables
	DiscoverColumns                  = introspection.DiscoverColumns
	postgresColumnsBasicStmt         = introspection.PostgresColumnsBasicStmt
	postgresConstraintsCountStmt     = introspection.PostgresConstraintsCountStmt
	postgresConstraintColumnsStmt    = introspection.PostgresConstraintColumnsStmt
	sqliteColumnsStmt                = introspection.SQLiteColumnsStmt
)
