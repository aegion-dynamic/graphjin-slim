...
// MatchRole is the minimal role surface needed to compile match expressions.
type MatchRole struct {
	Name  string
	Match string
}

// CompiledMatch is a role name bound to a compiled match expression.
type CompiledMatch = compiledRoleMatch

// CompileMatches compiles role match expressions.
func CompileMatches(roles []MatchRole) ([]CompiledMatch, error) {
	return compileRoleMatches(roles)
}

func compileRoleMatches(roles []MatchRole) ([]compiledRoleMatch, error) {
...
// Role returns the matched role name.
func (m compiledRoleMatch) Role() string { return m.role }

// Eval evaluates the match expression against attributes.
func (m compiledRoleMatch) Eval(attrs map[string]any) (bool, error) {
	return m.expr.eval(attrs)
}
